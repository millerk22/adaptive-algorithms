...
def _fetch_ucirepo(*args, **kwargs):
    try:
        ...
        raise ImportError("Install ucimlrepo to load UCI datasets.") from exc
    return fetch_ucirepo(*args, **kwargs)


def _graphlearning_tools():
    try:
        ...
        raise ImportError("Install graphlearning to load graphlearning-backed datasets.") from exc
    return load, generate, wm, graph
...
def load_dataset(dataset_name, n_test=500):
    if dataset_name == "blobssmallest":
        ...
        data = _fetch_ucirepo(id=332)
        X = data.data.features
        labels = data.data.targets
        print(X.shape)
    elif dataset_name == "adult":
        data = _fetch_ucirepo(id=20)
        X = data.data.features
        labels = data.data.targets
        print(X.shape)
    elif dataset_name == "yalefaces":
        X, labels = np.load(_data_path("yalefaces.npz"), allow_pickle=True).values()
        print(X.shape)
    elif dataset_name == "interactions":
        data = loadmat(_data_path("interactions.mat"))
        X = data['B'].T.astype(np.float64)
        labels = None
    elif dataset_name == "bfi":
        X, labels = np.load(_data_path("bfi.npz"), allow_pickle=True).values()
        ...
        data = np.load(_data_path("urban.npz"))
        X = data['H']
        X = 1.0*X
        X -= min(0.0, X.min())
        X /= np.max(np.max(X))
        ...
        X = np.load(_data_path("salinasa.npz"))['H']
        X = np.reshape(X, (X.shape[0]*X.shape[1], X.shape[2]))
        labels = np.load(_data_path("salinasa.npz"))['labels']
        ...
        X = np.load(_data_path("pavia.npz"))['H']
        X = np.reshape(X, (X.shape[0]*X.shape[1], X.shape[2]))
        labels = np.load(_data_path("pavia.npz"))['labels']
        labels = np.reshape(labels, (labels.shape[0]*labels.shape[1], 1)).flatten()
        mask = labels != 0
        X = X[mask]
        X = 1.0 * X
        ...
        labels = np.load(_data_path("snps/labels.npy"), allow_pickle=True)
        X = np.load(_data_path("snps/data.npy"), allow_pickle=True)
        ...
        load, generate, _, _ = _graphlearning_tools()
        X, labels = load("mnist", metric="vae")
        inds = generate(labels, rate=1000, seed=42)
        X = X[inds]
        labels = labels[inds]
    elif dataset_name == "mnistw":
        X, labels = np.load(_data_path("mnist_W.npz")).values()
    elif dataset_name == "mnistsc":
        load, generate, wm, graph = _graphlearning_tools()
        ...
        load, _, _, _ = _graphlearning_tools()
        X, labels = load("mnist", metric="raw")
        X = X.T
        labels = None 
